...
sub get_latest_backup {
    my ($path, $name, $suffix) = @_;
    my $pattern = $path . "." . $name . $suffix . ".butr.*";
...
sub backup_file {
    ...
    my $latest_backup = get_latest_backup($path, $name, $suffix);
    ...
    my $version = get_next_version($path, $name, $suffix);

    # Create backup filename (.filename.butr.v1.YYYYMMDD_HHMMSS)
    my $backup_file = sprintf("%s.%s%s.butr.v%d.%s",
        $path, $name, $suffix, $version, $timestamp);
...
sub recover_file {
    ...
    my $backup_pattern = $path . "." . $name . $suffix . ".butr.*";
...
Notes:
    ...
    - Backup files are hidden (start with .)
